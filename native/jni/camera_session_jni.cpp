// Berxel 相机统一 JNI 入口：把 native/camera 抽象接到 Kotlin NativeBridge。
...
// 进程内全局注册表仅保留 Berxel；VIN 双相机驱动由 vin-capture AAR 管理。
...
// Berxel 相机使用已关联的 USB fd；configJson 透传给 driver(SessionConfig.options_json)。
...
// 厂商扩展诊断统计(driver 自定义 int64 序列)。无扩展返长度 0 数组。
