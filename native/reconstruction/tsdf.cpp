...
TsdfStats TsdfVolume::Stats() const {
    TsdfStats s{};
    // grid_dim^3 大网格（如 200^3=8M、512^3=134M）整型乘法会溢出（UB），用 size_t 算；
    // TsdfStats::allocated_voxels/integrated_voxels 已 widen 到 int64_t（tsdf.h），回写不截断。
    const std::size_t total_voxels =
        static_cast<std::size_t>(grid_dim_) * grid_dim_ * grid_dim_;
    s.allocated_voxels = static_cast<int64_t>(total_voxels);
