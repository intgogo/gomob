...
- **M12.2 修掉前禁并行跑服务端 harness** (vinref_lifecycle / cv_vin_compare / cv_vin_pipeline / cv_vinref_compare / vinref_compare_quality): 硬编码同组端口 + 对共享 gomob-pg 全表 DELETE + redis FLUSHDB, 并行互杀且清开发者本机库。证据: TODO M12.2。
...
- **M11.A 接线/部署必做**: M11.1 P100R3 出厂内参 blob JNI 下发 (CRIT, 归 berxel 模块但欠账挂此处出账); M11.2 laserstationweb 强口令部署变量; M11.5 Room migration 测试进 CI; M11.6 https/wss TLS 入口。
...
- **M12 遗留**: M12.2 服务端 harness 端口/库隔离 (07-14 事故后优先级更高); M12.3 native 断言去 NDEBUG 依赖 + eys3d smoke 永真占位换真判据; M12.5 固定 sleep → 轮询健康检查 + curl --max-time; M12.6 scan_bundle_roundtrip 改用真产物校验契约。
