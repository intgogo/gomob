...
- [**官方 SDK 可逆向 + 设备协商深度帧 1026584≠我方 513280 2026-06-03**](finding_berxel_sdk_acquisition_re_2026-06-03.md) — 官方 `libBerxelUvcDriver.so` 未混淆(完整 C++ 符号),`BerxelStreamImpl::addData`=帧组装核心、`BerxelDeviceSonix`/`HostProtocolSonix`=Sonix 取流、`BerxelDepthProcessor::processDepth`=后处理,`nm -DC`+`llvm-objdump` 可重建。**设备协商 depth frameSize=1026584=2×513292(=513280+12B header),color=512000,我方硬编码 513280 且丢弃协商值** → 盲切疑似把 [depth|IR] 拼帧劈开=bad_marker 73%/帧率抖真因(待 DUMP 验)。校准:marker 0x0600/0x0500 分流是官方正解、硬件温补已开(**别加软温补**)、401→400 末状态行一致;终态=按真实帧长组帧+marker 拆面,废 513280 盲切。
