...
> 当前生产实现已迁移到 `vendor/vin-rubbing`：Android AAR 坐标为
> `io.vinrubbing:vin-capture`，服务入口为 `POST /v1/vin/process`。本文件的早期端侧方案和
> 旧 cvengine 路径保留作历史决策依据，不能作为 gomob 当前生产接口或驱动实现。
