...
|     | M-S10.2a 完整业务端点：迁 ivv 的 `RequestVINDetect`（含 ProcVINDet yolo 检测整流程）/ `RequestVinCompare`（含双图检测）/ 等；gin → net/http ServeMux 改写 | 留 Phase 2.3 后续；地基已就绪：core 模型加载 + vin-ref 客户端 + judge/proc 算法都在；缺一个把"整张 VIN 图"切成 17 字符 mask 的检测器接入 | `02-api-contract.md` §14 |
