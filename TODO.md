...
|     | M-S10.2a 完整业务端点：迁 ivv 的 `RequestVINDetect`（整流程含 GetVinRotatedRects/cropRRect/双行/弧形）/ `RequestVinCompare` | 地基已铺平（core 模型加载 + judge/proc 算法 + vinrefclient + yolo VMASK），缺只是把整张图 → 17 字符 mask 的拼接逻辑迁过来 | `02-api-contract.md` §14 |
