...
|     | M-S10.2a 完整业务端点：迁 ivv 的 `RequestVINDetect`（含 ProcVINDet 模型推理）/ `RequestVinCompare`（含双图检测）/ 等；gin → net/http ServeMux 改写 | 留 Phase 2.x；ivv 文件 27 个，需先迁 `apps/core` + `apps/data` 替代为 gomob `pkg/repo` | `02-api-contract.md` §14 |
