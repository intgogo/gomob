...
const (
	...
	Y_M_D_H_I_S_3       = "2006-01-02T15:04:05"
	H_I_S               = "15:04:05"
)
...
func FormatTimeStruct(str interface{}, layout string) (time.Time, error) {
	...
	case string:
		//如果是字符串则转换成 标准日期时间格式
		date, err = time.ParseInLocation(layout, str.(string), loc)
		if err != nil {
...
//时间格式化
func TimeFormatByYmdHms(year, month, day, hour, min, sec int) time.Time {
	return time.Date(year, time.Month(month), day, hour, min, sec, 0, time.Local)
}

//当前年月日
func NowYearMonthDay() (year, month, day int) {
	a, b, c := time.Now().Date()
	year = a
	month = int(b)
	day = c
...
func ParseDate(s string) (year, month, day int) {
	...
	year = conv.StrToInt(ss[0])
	month = conv.StrToInt(ss[1])
	day = conv.StrToInt(ss[2])
	return
}

func ParseTime(s string) (hour, min, sec int) {
	ss := strings.Split(s, ":")
	if len(ss) != 3 {
		return
	}
	hour = conv.StrToInt(ss[0])
	min = conv.StrToInt(ss[1])
	sec = conv.StrToInt(ss[2])
	return
}
