...
double ContourDist(Points con, Point point);
struct RotatedRect CreateRotatedRect(Point2f center, Size size, float angle);
...
void DrawEllipse(Mat img, Point center, Point axes, double angle, double
             startAngle, double endAngle, Scalar color, int thickness);
void Line(Mat img, Point pt1, Point pt2, Scalar color, int thickness);
void DrawRectangle(Mat img, Rect rect, Scalar color, int thickness);
...
double ResizePadding(Mat src, Mat dst, Size size, Scalar padding, int interp);
...
void ConcatAffineMatrix(Mat first, Mat second, Mat out);
...
Mat FindHomography(Mat src, Mat dst, int method, double ransacReprojThreshold);
...
void FitLineM(Mat pts, Mat line, int distType, double param, double reps, double aeps);
...
void DetectROI(Mat img, Contours *outContours, RotatedRects *outRR);
int GetRotatedRectsIntersect(Points ra, Points rb, RotatedRect *out);
void ExtractMattingBackColor(Mat img, Mat matting, int ncolor, IntVector *outHSV);
...
void MatchMattingImage(Mat base, Mat src, MatchResult *res);

void EnhanceContrastU8C1(Mat img, float ratio);

void SplitTextLineImage(Mat input, Mats *output, int targetHeight, int maxWidth);

void BlendMask(const Mat image, const Mat mask, Mat blended_image, int reverse, float factor);

float CalFDSimilarities(Mat img1, Mat img2, int numDescriptors);
