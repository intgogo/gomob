...
// !!!golang!!!must release mat later
Mat Mats_get(struct Mats mats, int i);
struct DMatches MultiDMatches_get(struct MultiDMatches mds, int index);

struct ByteVector toByteVector(const char* buf, int len);

Size *Size_New(int width, int height);
Scalar *Scalar_New(double val1, double val2, double val3, double val4);

void Contours_Release(struct Contours cs);
void KeyPoints_Release(struct KeyPoints ks);
void Rects_Release(struct Rects rs);
void Mats_Release(struct Mats mats);
void Point_Release(struct Point p);
void Points_Release(struct Points ps);
void RotatedRect_Release(RotatedRect r);
void RotatedRects_Release(RotatedRects rs);
void DMatches_Release(struct DMatches ds);
void MultiDMatches_Release(struct MultiDMatches mds);
...
Mat Mat_NewFromBytes(int rows, int cols, int type, struct ByteVector buf);
...
struct ByteVector Mat_ToBytes(Mat m);
struct ByteVector Mat_DataPtr(Mat m);
...
void Mat_Set8UC3(Mat m, int row, int col, uint8_t c1, uint8_t c2, uint8_t c3);
...
void Point2fVector_Append(Point2fVector pfv, Point2f pf);
