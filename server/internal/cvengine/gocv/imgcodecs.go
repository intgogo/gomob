...
func IMEncode(fileExt FileExt, img Mat) (buf []byte, err error) {
	...
	if b.data == nil || b.length == 0 {
		C.ByteVector_Release(b)
		return nil, fmt.Errorf("IMEncode failed")
	}

	buf = C.GoBytes(unsafe.Pointer(b.data), C.int(b.length))
