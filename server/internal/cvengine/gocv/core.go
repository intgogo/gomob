...
func NewMatFromBytes(rows int, cols int, mt MatType, data []byte) (Mat, error) {
	if len(data) == 0 {
		return NewMat(), ErrEmptyByteSlice
	}

	cBytes := toByteVector(data)
...
func (m *Mat) Size() (dims []int) {
	...
	defer C.IntVector_Release(cdims)
...
// OpenCV Mat object.
func (m *Mat) Region(roi image.Rectangle) Mat {
	if roi.Min.X < 0 {
		roi.Min.X = 0
	}
	if roi.Min.Y < 0 {
...
func (m *Mat) GetIntAt(row int, col int) int {
	return int(C.Mat_GetInt(m.p, C.int(row), C.int(col)))
}

// GetIntAt3 returns a value from a specific x, y, z coordinate location
// in this Mat expecting it to be of type int aka CV_32S.
func (m *Mat) GetIntAt3(x, y, z int) int {
	return int(C.Mat_GetInt3(m.p, C.int(x), C.int(y), C.int(z)))
}
...
func (m *Mat) Set8UC3At(row int, col int, c1, c2, c3 uint8) {
	C.Mat_Set8UC3(m.p, C.int(row), C.int(col), C.uint8_t(c1), C.uint8_t(c2), C.uint8_t(c3))
}
...
// Append appends an image.Point at end of the PointVector.
func (pfv Point2fVector) Append(point2f Point2f) {
	p := C.struct_Point2f{
		x: C.float(point2f.X),
		y: C.float(point2f.Y),
	}

	C.Point2fVector_Append(pfv.p, p)
...
func toGoInts(vv C.IntVector) (dd []int) {
	h := &reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(vv.val)),
		Len:  int(vv.length),
		Cap:  int(vv.length),
	}
	pvv := *(*[]C.int)(unsafe.Pointer(h))

	dd = make([]int, int(vv.length))
	for i := 0; i < int(vv.length); i++ {
		dd[i] = int(pvv[i])
	}
...
func toGoLongs(vv C.IntVector) (dd []int64) {
	h := &reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(vv.val)),
		Len:  int(vv.length),
		Cap:  int(vv.length),
	}
	pvv := *(*[]C.long)(unsafe.Pointer(h))

	dd = make([]int64, int(vv.length))
	for i := 0; i < int(vv.length); i++ {
		dd[i] = int64(pvv[i])
	}
...
func toGoFloats(vv C.FloatVector) (dd []float32) {
	h := &reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(vv.val)),
		Len:  int(vv.length),
		Cap:  int(vv.length),
	}
	pvv := *(*[]C.float)(unsafe.Pointer(h))

	dd = make([]float32, int(vv.length))
	for i := 0; i < int(vv.length); i++ {
		dd[i] = float32(pvv[i])
	}
...
func toGoStrings(vv C.CStrings) (dd []string) {
	h := &reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(vv.strs)),
		Len:  int(vv.length),
		Cap:  int(vv.length),
	}
	pvv := *(*[](*C.char))(unsafe.Pointer(h))

	dd = make([]string, int(vv.length))
	for i := 0; i < int(vv.length); i++ {
		dd[i] = C.GoString(pvv[i])
	}

	return
}

func toGoBytes(b C.ByteVector) []byte {
	return C.GoBytes(unsafe.Pointer(b.data), b.length)
}

func toGoRects(ret C.Rects) []image.Rectangle {
	cArray := ret.rects
...
func toGoPoints(ret C.Points) []image.Point {
	cArray := ret.points
	length := int(ret.length)
	hdr := reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(cArray)),
	...
	pp := *(*[]C.Point)(unsafe.Pointer(&hdr))

	pts := make([]image.Point, length)
	for i, pt := range pp {
		pts[i] = image.Point{X: int(pt.x), Y: int(pt.y)}
	}
	return pts
}

func toGoContours(ret C.Contours) [][]image.Point {
	cArray := ret.contours
...
func toGoRotatedRect(ret C.RotatedRect) RotatedRect {
	return RotatedRect{
		Contour:      toGoPoints(ret.pts),
		BoundingRect: toGoRect(ret.boundingRect),
		Center:       image.Pt(int(ret.center.x), int(ret.center.y)),
		Width:        int(ret.size.width),
		Height:       int(ret.size.height),
		Angle:        float64(ret.angle),
	}
}

func toGoRotatedRects(ret C.RotatedRects) []RotatedRect {
	cArray := ret.rects
	length := int(ret.length)
	hdr := reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(cArray)),
	...
	pp := *(*[]C.RotatedRect)(unsafe.Pointer(&hdr))

	rr := make([]RotatedRect, length)
	for i, p := range pp {
		rr[i] = toGoRotatedRect(p)
	}
	return rr
}

func toGoRect(rect C.Rect) image.Rectangle {
	return image.Rect(int(rect.x), int(rect.y), int(rect.x+rect.width), int(rect.y+rect.height))
}

func toByteVector(b []byte) *C.ByteVector {
	if len(b) == 0 {
		return &C.ByteVector{
...
func toCIntVector(vv []int) C.IntVector {
	if len(vv) == 0 {
		return C.IntVector{
			val:    (*C.int)(unsafe.Pointer(nil)),
			length: 0,
		}
	}
	vv32 := make([]int32, len(vv))
	for i, v := range vv {
		vv32[i] = int32(v)
	}

	return C.IntVector{
		val:    (*C.int)(unsafe.Pointer(&vv32[0])),
		length: C.int(len(vv32)),
	}
}

func toCFloatVector(vv []float32) C.FloatVector {
	if len(vv) == 0 {
		return C.FloatVector{
			val:    (*C.float)(unsafe.Pointer(nil)),
			length: 0,
		}
	}
	return C.FloatVector{
		val:    (*C.float)(unsafe.Pointer(&vv[0])),
		length: C.int(len(vv)),
	}
}

func toCPoints(points []image.Point) C.Points {
	if len(points) == 0 {
		return C.Points{
			points: (*C.Point)(unsafe.Pointer(nil)),
			length: 0,
		}
	}

	cPointSlice := make([]C.Point, len(points))
	for i, point := range points {
		cPointSlice[i] = C.Point{
			x: C.int(point.X),
			y: C.int(point.Y),
		}
	}
	...
	return C.Points{
		points: (*C.Point)(unsafe.Pointer(&cPointSlice[0])),
		length: C.int(len(points)),
	}
}

func toCRects(rr []image.Rectangle) C.Rects {
	if len(rr) == 0 {
		return C.Rects{
			rects:  (*C.Rect)(unsafe.Pointer(nil)),
			length: 0,
		}
	}

	cRectsSlice := make([]C.Rect, len(rr))
	for i, r := range rr {
		cRectsSlice[i] = C.Rect{
			x:      C.int(r.Min.X),
			y:      C.int(r.Min.Y),
	...
	return C.Rects{
		rects:  (*C.Rect)(unsafe.Pointer(&cRectsSlice[0])),
		length: C.int(len(rr)),
	}
}

// func toCStrings(strs []string) C.CStrings {
...
func toCStrings(strs []string) C.CStrings {
	if len(strs) == 0 {
		return C.CStrings{strs: nil, length: 0}
	}

	ptrSize := unsafe.Sizeof((*C.char)(nil))
	cArray := C.malloc(C.size_t(uintptr(len(strs)) * ptrSize))
	cArraySlice := (*[1 << 28]*C.char)(unsafe.Pointer(cArray))[:len(strs):len(strs)]
