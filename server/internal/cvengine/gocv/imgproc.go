...
func ApproxPolyDP(curve []image.Point, epsilon float64, closed bool) (approxCurve []image.Point) {
	...
	defer C.Points_Release(cApproxCurve)
...
func CreateRotatedRect(center, size image.Point, angle float64) RotatedRect {
	c := C.Point2f{x: C.float(center.X), y: C.float(center.Y)}
	s := C.Size{width: C.int(size.X), height: C.int(size.Y)}
	result := C.CreateRotatedRect(c, s, C.float(angle))

	defer C.Points_Release(result.pts)
	return RotatedRect{
...
func FitEllipse(points []image.Point) RotatedRect {
	...
	defer C.Points_Release(cRect.pts)
...
func ContourDist(pp []image.Point, pt image.Point) float64 {
	contour := toCPoints(pp)
	cpt := C.Point{x: C.int(pt.X), y: C.int(pt.Y)}
	return float64(C.ContourDist(contour, cpt))
}
...
func Ellipse(img *Mat, center, axes image.Point, angle, startAngle, endAngle float64, c color.RGBA, thickness int) {
	...
	C.DrawEllipse(img.p, pc, pa, C.double(angle), C.double(startAngle), C.double(endAngle), sColor, C.int(thickness))
}
...
func Rectangle(img *Mat, r image.Rectangle, c color.RGBA, thickness int) {
	...
	C.DrawRectangle(img.p, cRect, sColor, C.int(thickness))
}
...
func ResizePadding(src Mat, dst *Mat, sz image.Point, padding Scalar, interp InterpolationFlags) float64 {
	cSize := C.struct_Size{
		width:  C.int(sz.X),
		height: C.int(sz.Y),
	}
	cPadding := C.struct_Scalar{
		val1: C.double(padding.Val1),
		val2: C.double(padding.Val2),
		val3: C.double(padding.Val3),
		val4: C.double(padding.Val4),
	}

	return float64(C.ResizePadding(src.p, dst.p, cSize, cPadding, C.int(interp)))
}
...
func ConcatAffineMatrix(first, second, dst Mat) {
	C.ConcatAffineMatrix(first.p, second.p, dst.p)
}
...
func FindHomography(srcPoints Mat, targetPoints Mat, method HomographyMethod, ransacReprojThreshold float64) Mat {
	return newMat(C.FindHomography(srcPoints.Ptr(), targetPoints.Ptr(), C.int(method), C.double(ransacReprojThreshold)))
}
...
func FitLineM(pts Mat, line *Mat, distType DistanceTypes, param, reps, aeps float64) {
	C.FitLineM(pts.p, line.p, C.int(distType), C.double(param), C.double(reps), C.double(aeps))
}

// MergeEvenImagesTo2Rows 无缩放拼接偶数张图片为2行布局（上n张、下n张，n=总张数/2）
// 要求：每行内所有图片高度一致，每列对应位置图片宽度一致
func MergeEvenImagesTo2Rows(imgs []Mat) (*Mat, error) {
	// 1. 基础校验：非空、偶数张
	if len(imgs) == 0 {
		return nil, fmt.Errorf("输入图片数量不能为空")
	}
	if len(imgs)%2 != 0 {
		return nil, fmt.Errorf("输入图片数量必须为偶数，实际传入%d张", len(imgs))
	}

	// 2. 分割上下两行（每行n张，n=总张数/2）
	rowCount := len(imgs) / 2
	topRowImgs := imgs[:rowCount]    // 上半行图片
	bottomRowImgs := imgs[rowCount:] // 下半行图片

	// 3. 校验行列尺寸合法性
	// 3.1 校验上下行图片数量匹配
	if len(topRowImgs) != len(bottomRowImgs) {
		return nil, fmt.Errorf("上下行图片数量不匹配（上%d张，下%d张）", len(topRowImgs), len(bottomRowImgs))
	}
	// 3.2 校验每行内所有图片高度一致，每列图片宽度一致
	var (
		topRowHeight    int // 上半行图片高度（所有行内图片高度需一致）
		bottomRowHeight int
		colWidths       []int // 每列图片的宽度（上下列需一致）
	)
	// 校验上半行
	for i, img := range topRowImgs {
		if img.Empty() {
			return nil, fmt.Errorf("上半行第%d张图片为空", i)
		}
		// 校验行内高度一致
		if i == 0 {
			topRowHeight = img.Rows()
		} else {
			if img.Rows() != topRowHeight {
				return nil, fmt.Errorf("上半行第%d张图片高度（%d）与行内高度（%d）不一致", i, img.Rows(), topRowHeight)
			}
		}
		// 记录每列宽度
		colWidths = append(colWidths, img.Cols())
	}
	// 校验下半行
	for i, img := range bottomRowImgs {
		if img.Empty() {
			return nil, fmt.Errorf("下半行第%d张图片为空", i)
		}
		// 校验行内高度一致
		if i == 0 {
			bottomRowHeight = img.Rows()
		} else {
			if img.Rows() != bottomRowHeight {
				return nil, fmt.Errorf("下半行第%d张图片高度（%d）与行内高度（%d）不一致", i, img.Rows(), bottomRowHeight)
			}
		}
		// 校验列宽度一致
		if img.Cols() != colWidths[i] {
			return nil, fmt.Errorf("第%d列图片宽度不匹配（上%d，下%d）", i, colWidths[i], img.Cols())
		}
	}
	...
	finalHeight := topRowHeight + bottomRowHeight

	// 5. 创建最终画布（与第一张图同类型、同通道）
	finalMat := NewMatWithSize(finalHeight, finalWidth, imgs[0].Type())
	if finalMat.Empty() {
		return nil, fmt.Errorf("创建最终画布失败")
	}
	...
	for i, img := range topRowImgs {
		...
		roiRect := image.Rect(
			currentX,              // x起始
			0,                     // y起始（上半行y=0）
			currentX+colWidths[i], // x结束
			topRowHeight,          // y结束
		)
		roi := finalMat.Region(roiRect)
		img.CopyTo(&roi)
		roi.Release() // 释放ROI引用
...
func EnhanceGrayImage(src Mat) {
	C.EnhanceGrayImage(src.p)
}

func GetRotatedRectsIntersect(ra, rb []image.Point) RotatedRect {
	a := toCPoints(ra)
	b := toCPoints(rb)
	c := C.RotatedRect{}
	ret := int(C.GetRotatedRectsIntersect(a, b, &c))
	if ret < 0 {
		return RotatedRect{}
	}
	return toGoRotatedRect(c)
}

func DetectROI(src Mat) (contours [][]image.Point, rrects []RotatedRect) {
	cContours := C.Contours{}
	cRR := C.RotatedRects{}

	C.DetectROI((C.Mat)(src.p), &cContours, &cRR)
...
func ExtractMattingBackColor(img, matting Mat, ncolor int, outHSV []int) {
	cHSV := C.IntVector{}
	C.ExtractMattingBackColor(img.p, matting.p, C.int(ncolor), &cHSV)
	outHSV = toGoInts(cHSV)
}
...
func MatchMattingImage(base, src Mat, res *MatchResult) {
	cRes := C.MatchResult{
		compMat:     res.Comp.p,
		compMatReal: res.CompReal.p,
	}

	C.MatchMattingImage(base.p, src.p, &cRes)

	res.BestX = int(cRes.bestX)
	res.BestY = int(cRes.bestY)
	res.BestMatch = float32(cRes.bestMatch)
	res.BestMatchReal = float32(cRes.bestMatchReal)
	res.DD = toGoInts(cRes.dd)
	res.PP = toGoContours(cRes.pp)

	C.IntVector_Release(cRes.dd)
	C.Contours_Release(cRes.pp)
}

func EnhanceContrastU8C1(img Mat, ratio float32) {
	C.EnhanceContrastU8C1(img.p, C.float(ratio))
}

func SplitTextLineImage(img Mat, targetHeight, maxWidth int) []Mat {
	cMats := C.Mats{}
	defer C.Mats_Release(cMats)
	C.SplitTextLineImage(img.p, &(cMats), C.int(targetHeight), C.int(maxWidth))

	outs := make([]Mat, cMats.length)
	for i := C.int(0); i < cMats.length; i++ {
		outs[i].p = C.Mats_get(cMats, i)
	}

	return outs
}

func BlendMask(img, mask, blendImage Mat, reverse int32, factor float32) {
	C.BlendMask(img.p, mask.p, blendImage.p, C.int(reverse), C.float(factor))
}

func CalFDSimilarities(img1, img2 Mat, numDescriptors int) float64 {
	return float64(C.CalFDSimilarities(img1.p, img2.p, C.int(numDescriptors)))
}
