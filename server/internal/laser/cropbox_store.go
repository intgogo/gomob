...
// CropBoxStore 持久化每装机点(bayKey，默认 unit_a_ip)每单元(unit a|b)的裁剪框。
type CropBoxStore interface {
	// GetCropBox 取某单元框；未设置返回 ok=false（非错误）。
...
func (s *DBCropBoxStore) GetCropBox(ctx context.Context, bayKey, unit string) (CropBox, bool, error) {
	raw, err := s.repo.Get(ctx, bayKey, unit)
...
func (s *DBCropBoxStore) SaveCropBox(ctx context.Context, bayKey, unit string, box CropBox) error {
