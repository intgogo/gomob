...
func (s *fakeCropBoxStore) GetCropBox(_ context.Context, k string) (CropBox, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	b, ok := s.box[k]
	return b, ok, nil
}
func (s *fakeCropBoxStore) SaveCropBox(_ context.Context, k string, b CropBox) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.box[k] = b
	return nil
